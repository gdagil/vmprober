...
func NewICMPProbe(config *ICMPConfig) interfaces.Probe {
	...
	// Safe conversion: seqStart is guaranteed to be in [0, 65535] range
	var seqVal uint16
	if seqStart < 0 {
		seqVal = 0
	} else if seqStart > 65535 {
		seqVal = 65535
	} else {
...
func (p *ICMPProbe) Execute(ctx context.Context, target types.Target) (*types.ProbeResult, error) {
	...
	// Safe conversion: masking with 0xFFFF ensures value is in [0, 65535] range
	maskedPID := pid & 0xFFFF
	var icmpID uint16
	if maskedPID < 0 {
		icmpID = 0
	} else if maskedPID > 65535 {
		icmpID = 65535
	} else {
		icmpID = uint16(maskedPID)
	}
	p.mu.Lock()
	p.sequence++
	if p.sequence == 0 {
